...
class TestFeedPage:
    ...
    def test_feed_all_orders_counter(self, driver):
        ...
        count = order_page.get_feed_count_all()
        create_order()
        assert int(order_page.get_feed_count_all()) > int(count)

    @allure.title('Проверяем счетчик заказов за сегодня')
    def test_feed_today_orders_counter(self, driver):
        order_page = OrderPage(driver)
        order_page.open_feed_page()
        count = order_page.get_feed_count_today()
        create_order()
        assert int(order_page.get_feed_count_today()) > int(count)
    ...
    def test_feed_in_work_id(self, driver):
        ...
        assert order_id == int(order_page.get_feed_in_work_id())